app.controller('unRegisteredStudentDetailsByMasterCtrl', ['MasterService', '$scope', '$rootScope', '$timeout', '$uibModalInstance',
    function (MasterService, $scope, $rootScope, $timeout, $uibModalInstance) {
        ...
        $scope.submit = function () {
            if ($scope.buffer.startDate && $scope.buffer.endDate) {
                window.open('/report/UnRegisteredStudentDetailsByMaster/'
                    + $scope.buffer.master.id + "?"
                    + "startDate=" + $scope.buffer.startDate.getTime() + "&"
                    + "endDate=" + $scope.buffer.endDate.getTime());
            } else {
                window.open('/report/UnRegisteredStudentDetailsByMaster/' + $scope.buffer.master.id);
            }
        };
