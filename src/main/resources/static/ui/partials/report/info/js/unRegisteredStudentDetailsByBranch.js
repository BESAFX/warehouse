app.controller('unRegisteredStudentDetailsByBranchCtrl', ['BranchService', '$scope', '$rootScope', '$timeout', '$uibModalInstance',
    function (BranchService, $scope, $rootScope, $timeout, $uibModalInstance) {
        ...
        $scope.submit = function () {
            if ($scope.buffer.startDate && $scope.buffer.endDate) {
                window.open('/report/UnRegisteredStudentDetailsByBranch/'
                    + $scope.buffer.branch.id + "?"
                    + "startDate=" + $scope.buffer.startDate.getTime() + "&"
                    + "endDate=" + $scope.buffer.endDate.getTime());
            } else {
                window.open('/report/UnRegisteredStudentDetailsByBranch/' + $scope.buffer.branch.id);
            }
        };
